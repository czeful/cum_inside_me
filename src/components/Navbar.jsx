...
const Navbar = () => {
  ...
  <div className="max-w-7xl mx-auto px-4 sm:px-8">
    <div className="flex justify-between h-16 items-center">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2 pr-2">
          <span className="hidden sm:block text-sm text-slate-600 font-semibold">
            {user?.Username || user?.Email?.split("@")[0] || 'User'}
          </span>
          <span className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-gradient-to-tr from-sky-100 via-emerald-100 to-teal-200 text-slate-700 font-bold shadow">
            {(user?.Username || user?.Email || "U").slice(0,2).toUpperCase()}
          </span>
        </div>
        <button
